...
class Fip(Resource):
    ...
    def register(self, config):
        ...
        _desc = (
            "By default, only FIPs not attached are considered. Include "
            "this flag to consider ALL fips"
        )
        self._sub_config.add_argument(
            "--with-attached", dest="with_attached", help=_desc, action="store_true"
        )
        _desc = (
            "Definition of a subnet within which the floating IP address"
            " must reside in order to be purged."
        )
        self._sub_config.add_argument(
            "--floating-subnet", dest="floating_subnet", help=_desc
        )
        _desc = (
            "Definition of a subnet within which the host's primary IP "
            "address must reside."
        )
        self._sub_config.add_argument(
            "--static-subnet", dest="static_subnet", help=_desc
        )
    ...
    def process(self):
        ...
        self._config.info("%d floating IPs passed address test" % len(self.__fips))
    ...
    def __filter_attached(self):
        ...
        if not force_attached:
            ...
            self._config.info("%d floating IPs passed attached test" % len(self.__fips))
            self.__debug_fips()
    ...
    def __filter_by_address(self):
        ...
        if floating_subnet is not None:
            __fips = filter(
                self.__filter_factory("floating_ip_address", floating_subnet),
                self.__fips,
            )
        ...
        if static_subnet is not None:
            __fips = filter(
                self.__filter_factory("fixed_ip_address", static_subnet), self.__fips
            )
            self.__fips = list(__fips)
    ...
    def send_emails(self):
        """Send emails to someone if they want it."""
